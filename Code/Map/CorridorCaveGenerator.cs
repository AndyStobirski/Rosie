...
namespace Rosie.Map
{
    ...
    /// </summary>
    class CorridorCaveGenerator : Generator
    {

        public int FloorTile = 1378;
        ...
        private void BuildCaves()
        {
            ...
            for (int x = 0; x <= Iterations; x++)
            {
                ...
                if (Neighbours_Get1(cell).Count(n => Point_Get(n) > 0) > Neighbours)
                    Point_Set(cell, FloorTile);
                else
                    Point_Set(cell, 0);
            }
            ...
            for (int ctr = 0; ctr < 5; ctr++)
            {
                ...
                for (int x = 0; x < MapSize.Width; x++)
                    for (int y = 0; y < MapSize.Height; y++)
                    {
                        ...
                        if (
                                Point_Get(cell) > 0
                                && Neighbours_Get(cell).Count(n => Point_Get(n) == 0) >= EmptyNeighbours
                            )
                            Point_Set(cell, 0);
                    }
            }
            ...
            for (int x = 0; x < MapSize.Width; x++)
                for (int y = 0; y < MapSize.Height; y++)
                {
                    ...
                    if (
                            Point_Get(cell) == 0
                            && Neighbours_Get(cell).Count(n => Point_Get(n) == 1) >= EmptyCellNeighbours
                        )
                        Point_Set(cell, FloorTile);
                }
        }
        ...
        private List<Point> Corridor_Attempt(Point pStart, Point pDirection, bool pPreventBackTracking)
        {
            ...
            while (pTurns >= 0)
            {
                ...
                while (corridorlength > 0)
                {
                    ...
                    if (Point_Check(pStart) && Point_Get(pStart) > 0)
                    {
                        lPotentialCorridor.Add(pStart);
        ...
        {
            if (val == 0)
                Level.Map[p.X, p.Y] = null;
            else
                Level.Map[p.X, p.Y] = new Floor(val);
        }
        ...
        {
            if (Level.Map[p.X, p.Y] != null)
                return Level.Map[p.X, p.Y].Gfx;
            else
                return 0;
        }
