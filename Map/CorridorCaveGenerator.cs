...
namespace Rosie.Map
{
    ...
    class CorridorCaveGenerator : Generator
    {
        ...
        {
            return Library.Directions.Select(d => new Point(p.X + d.X, p.Y + d.Y))
        ...
        {
            return Library.Directions1.Select(d => new Point(p.X + d.X, p.Y + d.Y))
                    .Where(d => Point_Check(d)).ToList();
        }
        ...
        private Point Direction_Get(Point p)
        {
            ...
            do
            {
                newdir = Library.Directions[_rnd.Next(0, Library.Directions.Count())];
