...
namespace Rosie.Map
{
    ...
    /// 
    /// Waypoints, which are used for creature navigation are added in thw methods
    /// Corridor_Build() and Room_Build()
    /// </summary>
    class MapGenerator : Generator
    {
        ...
        private void Room_GetEdge(out Point pLocation, out Point pDirection)
        {
            ...
            pDirection = Library.Directions[_rnd.Next(0, Library.Directions.GetLength(0))];
        ...
        private void Corridor_GetEdge(out Point pLocation, out Point pDirection)
        {
            ...
            do
            {
                ...
                foreach (Point p in Library.Directions)
        ...
        {
            //get the middle of the corridor
            Point mid = lPotentialCorridor[lPotentialCorridor.Count() / 2];

            WayPoint wp = new WayPoint
            {
                X = mid.X,
                Y = mid.Y,
                Iam = Enums.WayPoints.Corridor,
                ID = lPotentialCorridor.Count()
            };
            Level.wayPoints.Add(wp);
        ...
        private Point Direction_Get(Point pDir)
        {
            ...
            do
            {
                NewDir = Library.Directions[_rnd.Next(0, Library.Directions.GetLength(0))];
        ...
        private void Room_Build()
        {
            ...
            WayPoint wp = new WayPoint
            {
                X = rctCurrentRoom.X + rctCurrentRoom.Width / 2,
                Y = rctCurrentRoom.Y + rctCurrentRoom.Height / 2,
                Iam = Enums.WayPoints.Room,
                ID = rctBuiltRooms.Count()
            };
            Level.wayPoints.Add(wp);
        ...
        /// <param name="x"></param>
        /// <param name="y"></param>
        private void Point_Set(int x, int y)
        {
            Level.Map[x, y] = new Floor();
        }
